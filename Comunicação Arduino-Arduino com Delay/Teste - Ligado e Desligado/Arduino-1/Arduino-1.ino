  ...
  void setup()
  {
    ...
    // Seta as portas digitais de entrada de dados.
  ...
  void loop()
  {
    ...
    Time = millis();
    // Verifica a posição do interruptor.
    if(digitalRead(POWER_INT) == HIGH) RobotOn = true;
    else RobotOn = false;
    ...
    // Verifica atualizações nas variáveis de estado.
    if (_RobotOn != RobotOn) { AddMessage(2, 1, (int) RobotOn); _RobotOn = RobotOn; }
